...
Let's try to make a working virtual environment for our `pkoffee` project. We can use the pixi command line interface to initialize a workspace and add dependencies:
```bash
...
!!! success "We managed to install `pkoffee` dependencies in a virtual environment!"
...
There is one issue with the above environments definitions: `prod` and `default` have a set of dependencies in common, however nothing enforces that the versions of dependencies solved for `prod` are the same than for `default`. This can lead to surprising results, for instance: `pkoffee` could work with the `prod` environment, but not with the `default` environment, because the `numpy` version in `default` is different than in `prod`. In order to avoid this surprising behavior, pixi implements "solve groups": environments belonging to the same solve group are solved jointly, ensuring that the versions in all environments are consistent. We can update our manifest to request that the `prod` and the `default` environment be solved jointly:
...
and remove them from the `default feature`:
```diff
[dependencies]
numpy = ">=2.4.1,<3"
matplotlib = ">=3.10.8,<4"
...
Our environment definitions now look read:
```toml
[dependencies]
numpy = ">=2.4.1,<3"
matplotlib = ">=3.10.8,<4"
...
```
We have 5 features: the `default` feature with `pkoffee` dependencies, but `python` version not set. The `dev` feature with `jupyterlab`, and 3 `python` feature, each with a specific python version. We combine these features to form 4 environments: the `default` environment with `python` 3.13, `pkoffee`'s dependencies + `jupyterlab`, and 3 "prod" environments with `pkoffee`'s dependencies, no additional development tool, and a specific `python` version. This may seem completely overkill at the moment, but we will see in further lectures that those environments will be useful to automatically test our package with different python versions.
...
- [ty](https://open-vsx.org/vscode/item?itemName=astral-sh.ty) python language server and type checker. The `pylance` language server, used by default with VS Code, can not be used with VS Codium, so the _very new_ `ty` can be used a replacement. If using `ty` it is recommended to change the python language server setting of VS Codium to None (as `ty` will provide the language server).
- [Even Better Toml](https://open-vsx.org/vscode/item?itemName=tamasfe.even-better-toml): an extension for `.toml` language support.
- [autoDocstring - Python Docstring Generator](https://open-vsx.org/vscode/item?itemName=njpwerner.autodocstring): an extension to quickly generate docstrings. For scientific projects, we would recommend using the `numpy` docstring style.
...
- [`uv`](https://docs.astral.sh/uv/) is the recommended tool for managing `pypi`-only projects and environments with reproducibility guarantees.
- [`pixi`](https://pixi.prefix.dev/latest/) is the recommended tool for managing non `pypi`-only projects with reproducibility guarantees, using the [`conda-forge`](https://conda-forge.org/) as default channel.
    - The `pixi.toml` configures a workspace, which can include several environments specification.
